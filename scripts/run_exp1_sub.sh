...
setup_env() {
  ...
  sleep 1
  sudo chronyc add server "${CHRONY_SERVER}" iburst 2>/dev/null || true
  sudo chronyc reload sources 2>/dev/null || true

  # 인터넷 NTP 소스 오프라인 → A만 사용
  sudo chronyc offline 2>/dev/null || true
  sudo chronyc online "${CHRONY_SERVER}" 2>/dev/null || true

  # iburst 완료 대기 후 스텝 동기화
  sleep 8
  sudo chronyc makestep 2>/dev/null || true

  echo "[setup] 클록 동기화 대기 (목표: |offset| < 0.5ms)..."
  ...
  for i in $(seq 1 60); do
    ...
    if [[ -n "${offset_sec}" ]]; then
      ...
      if awk "BEGIN{v=${offset_sec}+0; if(v<0)v=-v; exit !(v < 0.0005)}"; then
        echo ""
...
# 실험 종료 후 인터넷 NTP 소스 복구
restore_ntp() {
  if [[ -n "${CHRONY_SERVER}" ]]; then
    sudo chronyc online 2>/dev/null || true
  fi
}
