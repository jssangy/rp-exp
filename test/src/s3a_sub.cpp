...
static constexpr uint64_t EXPECTED = 40 * 60;  // 40 Hz × 60 s

class S3aSubscriber : public rclcpp::Node
{
public:
  S3aSubscriber()
  : Node("s3a_subscriber"), running_(true), measuring_(false), recv_count_(0)
  {
    rclcpp::QoS qos(1);
    qos.best_effort();
    ...
    RCLCPP_INFO(get_logger(), "S3-a subscriber started: BEST_EFFORT (10s warmup + 60s measure)");
  }
