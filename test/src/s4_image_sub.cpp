...
public:
  ...
  : Node("s4_image_subscriber"), running_(true), measuring_(false), recv_count_(0)
  {
    rclcpp::QoS qos(1);
    qos.best_effort();
    ...
    RCLCPP_INFO(get_logger(),
      "S4 image subscriber started: %s, BEST_EFFORT (10s warmup + 60s measure)",
      topic.c_str());
  }
    ...
    while (running_) {
      ...
      while (!queue_.empty()) {
        ...
        if (measuring_) {
          ...
          if (elapsed >= 5.0) {
            RCLCPP_INFO(get_logger(),
              "recv %lu msgs | rate %.1f Hz | %ux%u %s %.2f MB | latency avg %.2f ms max %.2f ms",
