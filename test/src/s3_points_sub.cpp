...
public:
  S3PointsSubscriber()
  : Node("s3_points_subscriber"), running_(true), measuring_(false), recv_count_(0)
  {
    rclcpp::QoS qos(1);
    qos.best_effort();
    sub_ = create_subscription<sensor_msgs::msg::PointCloud2>(
      "/points", qos,
      [this](sensor_msgs::msg::PointCloud2::SharedPtr msg) { enqueue(std::move(msg)); });

    proc_thread_ = std::thread(&S3PointsSubscriber::process_loop, this);
    ...
    RCLCPP_INFO(get_logger(), "S3 points subscriber started: BEST_EFFORT (10s warmup + 60s measure)");
  }
...
private:
  void enqueue(sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    while (running_) {
      ...
      while (!queue_.empty()) {
        ...
        if (measuring_) {
          ...
          if (elapsed >= 5.0) {
            RCLCPP_INFO(get_logger(),
              "recv %lu msgs | rate %.1f Hz | payload %u KB | latency avg %.2f ms max %.2f ms",
              recv_count_.load(), window_count / elapsed, last_row_step / 1024,
  ...
  std::queue<sensor_msgs::msg::PointCloud2::SharedPtr> queue_;
  std::mutex mtx_;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<S3PointsSubscriber>();
