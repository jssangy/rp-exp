...
static constexpr uint64_t EXPECTED = 200 * 60;  // 200 Hz × 60 s

class S2Subscriber : public rclcpp::Node
{
public:
  S2Subscriber()
  : Node("s2_subscriber"), running_(true), measuring_(false), recv_count_(0)
  {
    rclcpp::QoS qos(1);
    qos.best_effort();
    ...
    RCLCPP_INFO(get_logger(), "S2 subscriber started: BEST_EFFORT (10s warmup + 60s measure)");
  }
